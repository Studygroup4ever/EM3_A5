...
pacman::p_load(readxl,
               tidyverse, tidyr, lmerTest, lme4, metafor)
...
```{r}
# Removing NA's from HC_M and SD_HC_M (but I actually do not use it)
df1 <-
  df %>% filter(PITCH_F0_HC_M != is.na(PITCH_F0_HC_M) |
                  PITCH_F0SD_HC_M != is.na(PITCH_F0SD_HC_M)) %>%   select(
                    c(
                      'StudyID',
                      'PITCH_F0_HC_M',
                      'PITCH_F0_SZ_M',
...
```{r}
# # Making it long so we can use it for modelling - do we need to?
# long <-
#   tidyr::pivot_longer(df1, c(PITCH_F0_HC_M, PITCH_F0_SZ_M), values_to = 'Pitch_M') %>%
#   mutate(
#     Pitch_SD = tidyr::pivot_longer(df1, c(PITCH_F0SD_HC_M, PITCH_F0SD_SZ_M))[['value']],
